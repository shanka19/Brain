...
void Neuron::stepInTime1(Neurons *neurons)
{
  ...
  {
    //std::cout << "Now Entering StepInTime1()" << std::endl;
    double totalDistance=0;
    for (unsigned int i=0; i<neuralRelations_.size(); ++i) totalDistance+=neuralRelations_.at(i)->distance;
    for (unsigned int i=0; i<neuralRelations_.size(); ++i)
    {
      ...
      targetNeuron->receive(0.5*neuralRelations_.at(i)->distance/totalDistance);
      //std::cout << "targetNeuron potential: " << targetNeuron->potential_ << " Target Neuron : " << neuralRelations_.at(i)->index <<std::endl;
