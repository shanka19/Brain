...
void dtime_generation(std::string directory, TFile *file)
{
  TGraph *g_dtime_generation=(TGraph*)file->Get("g_dtime_generation");
  unsigned int nGenerations=g_dtime_generation->GetN();
  std::cout << nGenerations << std::endl;
  TH1F *h_dtime_generation=new TH1F("h_dtime_generation", "; generations; Average time to next meal (Crossover Type 1 + Mutation)", nGenerations, 0, nGenerations);
  ...
  h_dtime_generation->SetMinimum(500);
  ...
  c_dtime_generation->SaveAs((directory + "/c_dtime_generation.png").c_str());
  c_dtime_generation->SaveAs((directory + "/c_dtime_generation.root").c_str());
}
...
/*void dtime_generation(TFile *file)
{
  ...
  unsigned int nGenerations=g_dtime_generation->GetN();
  std::cout << nGenerations << std::endl;
  TH1F *h_dtime_generation=new TH1F("h_dtime_generation", "; generations; Average time to next meal (Crossover + Mutation)", nGenerations, 0, nGenerations);
  for (unsigned int i=0; i<nGenerations; ++i)
  {
    double x,y;
    g_dtime_generation->GetPoint(i, x, y);
  ...
  c_dtime_generation->SaveAs("c_dtime_generation.root");
}*/
...
void DisplayPlots(std::string directory)
{
  std::cout<< "Opening directory: "<< directory << std::endl;
  TFile *file=new TFile((directory + "/AnalyzeThis.root").c_str());
  ...
  dtime_generation(directory, file);
